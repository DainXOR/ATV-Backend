...
func (sessionType) Create(u models.SessionCreate) types.Result[models.SessionDBMongo] {
	...
	if studentResult.IsErr() {
		...
		logger.Error("Failed to get student by ID: ", httpErr)
	...
	if companionResult.IsErr() {
		...
		logger.Error("Failed to get companion by ID: ", httpErr)
		return types.ResultErr[models.SessionDBMongo](httpErr)
	}
	...
	extraInfo["IDCompanionSpeciality"] = companion.IDSpeciality.Hex()

	sessionDB := u.ToInsert(extraInfo)
	result, err := configs.DB.InsertOne(sessionDB)

	if err != nil {
		logger.Error("Error inserting session: ", err)
		return types.ResultErr[models.SessionDBMongo](err)
	}

	sessionDB.ID, err = models.DBIDFrom(result.InsertedID)
...
func (sessionType) GetByID(id string) types.Result[models.SessionDBMongo] {
	...
	var session models.SessionDBMongoReceiver
	...
	return types.ResultOk(session.ToDB())
}
func (sessionType) GetAllByStudentID(id string) types.Result[[]models.SessionDBMongo] {
	...
	usersR := models.SessionDBMongo{}.ReceiverList()
	...
	sessions := utils.Map(usersR, models.SessionDBMongoReceiver.ToDB)
